...
class SimpleKeyTeleop():
    ...
    def callstart_client(self):
        rospy.wait_for_service('/lip_walking_controller_ds/generate_fixed_plan')
        try:
            emptySrv = rospy.ServiceProxy('/lip_walking_controller_ds/generate_fixed_plan',std_srvs.srv.Empty)
            response = emptySrv()
            return True
